...
[![Share your answer](https://img.shields.io/badge/Share_your_answer-blue?style=flat)](https://github.com/tyankatsu0105/utility-challenges/issues/new?labels=answer,<%= category %>/<%= h.changeCase.camelCase(name) %>&template=answer.md&title=[answer+-+<%= category %>/<%= h.changeCase.camelCase(name) %>])
